...
function UserChip({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        'flex items-center gap-2 px-2 py-1.5 rounded-lg border bg-card text-sm',
        'cursor-grab active:cursor-grabbing select-none transition-all w-full',
        isDragging ? 'opacity-0' : 'hover:border-primary/50 hover:bg-accent',
      )}
    >
      <span {...attributes} {...listeners} className="touch-none shrink-0">
...
export function AssignUsersSheet({ open, role, onOpenChange }: AssignUsersSheetProps) {
  ...
  }, [allUsers, assigned, searchAvail, searchAssigned])
              ...
              <div className="flex-1 overflow-hidden grid grid-cols-2 gap-0 divide-x">
                ...
                <div className="flex flex-col overflow-hidden">
                  ...
                  <DroppablePanel id="available" isEmpty={available.length === 0} placeholder="Tất cả nhân sự đã được gán">
                    {available.map((u) => (
                      <button key={u.id} onClick={() => add(u.id)} className="group relative w-full text-left" title={`Gán: ${u.fullName}`}>
                        <UserChip user={u} containerId="available" />
                        <ArrowRight className="absolute right-2 top-1/2 -translate-y-1/2 h-3.5 w-3.5 bg-primary text-primary-foreground rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
                      </button>
                    ))}
                  </DroppablePanel>
                </div>
