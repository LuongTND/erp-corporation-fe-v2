...
import type { StoreHoursResponse } from '../../types/admin.types'
...
function initRows(data: StoreHoursResponse[]): DayRow[] {
...
export function StoreHoursDialog({ open, storeName, storeHours, isLoading, isSaving, onOpenChange, onSave }: Props) {
  const [rows, setRows] = useState<DayRow[]>(() => initRows(storeHours))

  useEffect(() => { setRows(initRows(storeHours)) }, [storeHours])

  const setRow = (idx: number, patch: Partial<DayRow>) =>
    setRows(prev => prev.map((r, i) => i === idx ? { ...r, ...patch } : r))

  const handleSave = () => {
    const origMap = Object.fromEntries(storeHours.map(h => [toDayIndex(h.dayOfWeek), h]))
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-0 p-0 sm:max-w-md data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-bottom-4 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=closed]:slide-out-to-bottom-2 animation-duration-250">
        <DialogHeader className="flex-row items-center gap-3 border-b px-5 py-4">
          <figure className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-blue-500/10">
            <Clock className="h-4 w-4 text-blue-500" />
          </figure>
          <hgroup>
            <DialogTitle className="text-base">Giờ mở cửa</DialogTitle>
        ...
        <DialogFooter className="mx-0 mb-0 rounded-b-xl border-t bg-muted/30 px-5 py-3">
          ...
          <Button size="sm" onClick={handleSave} disabled={isSaving}>
            {isSaving ? 'Đang lưu...' : 'Lưu giờ mở cửa'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
