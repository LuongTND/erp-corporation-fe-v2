  ...
  return (
    <button
      ...
      className={cn(
        ...
        !node.isActive && 'border-dashed opacity-70',
      )}
    >
      <div className="flex items-center gap-1 mb-0.5">
        <p className="text-[10px] font-mono text-muted-foreground leading-none flex-1 truncate">{node.departmentCode}</p>
        {!node.isActive && (
          <Badge variant="outline" className="text-[9px] px-1 h-3.5 text-muted-foreground border-dashed leading-none shrink-0">Vô hiệu</Badge>
        )}
      </div>
