...
export function KpiDeptPerformance() {
  return (
    <div className="bg-card rounded-xl shadow-sm p-6 h-full">
      <h3 className="text-sm font-semibold text-foreground mb-5">Avg. Score by Department</h3>
        ...
        {DEPTS.map((dept) => {
          ...
          return (
            <div key={dept.name} className="flex items-center gap-3">
              <span className="text-sm text-muted-foreground w-24 shrink-0">{dept.name}</span>
