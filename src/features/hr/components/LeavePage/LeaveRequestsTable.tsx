    ...
    <div className="bg-card rounded-xl shadow-sm overflow-auto max-h-full min-h-0">
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {filtered.map((leaveRequest) => (
              <TableRow key={leaveRequest.id} className="border-border hover:bg-card transition-colors">
                ...
                <TableCell className="pl-5 py-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-medium text-foreground leading-tight">{leaveRequest.employee.name}</p>
                      <p className="text-[10px] text-muted-foreground">{leaveRequest.employee.department}</p>
                    </div>
                  </div>
                </TableCell>
                ...
                {/* Leave Type */}
                <TableCell>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full whitespace-nowrap ${TYPE_BADGE[leaveRequest.type]}`}>
                    {leaveRequest.type}
                  </span>
                </TableCell>

                {/* From / To */}
                <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{leaveRequest.from}</TableCell>
                <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{leaveRequest.to}</TableCell>

                {/* Days */}
                <TableCell>
                  <span className="text-xs font-semibold text-foreground">{leaveRequest.days}d</span>
                </TableCell>

                {/* Reason */}
                <TableCell className="max-w-[140px]">
                  <p className="text-xs text-muted-foreground truncate" title={leaveRequest.reason}>{leaveRequest.reason}</p>
                </TableCell>

                {/* Applied On */}
                <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{leaveRequest.appliedOn}</TableCell>

                {/* Status */}
                <TableCell>
                  <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${STATUS_BADGE[leaveRequest.status]}`}>
                    {leaveRequest.status}
                  </span>
                </TableCell>
                ...
                <TableCell>
                  {leaveRequest.status === 'Pending' ? (
                    <div className="flex items-center gap-1.5">
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); setConfirmAction({ type: 'approve', reqId: leaveRequest.id }) }}
                      ...
                      <button
                        type="button"
                        onClick={(e) => { e.stopPropagation(); setConfirmAction({ type: 'decline', reqId: leaveRequest.id }) }}
                  ...
                  ) : leaveRequest.actionedBy ? (
                    <div>
                      <p className="text-[10px] text-muted-foreground">by {leaveRequest.actionedBy}</p>
                      <p className="text-[10px] text-muted-foreground">{leaveRequest.actionedAt}</p>
                    </div>
                  ) : (
